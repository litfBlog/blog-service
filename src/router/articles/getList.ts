 ...
 * @LastEditTime: 2022-03-15 18:29:13
 * @LastEditors: litfa
 * @Description: 首页文章
...
router.all('/home', async (req, res) => {
  ...
  const [err, results] = await query('SELECT a.*,a.*,b.username, b.`avatar` FROM `articles` a JOIN `users` b ON a.`author`=b.id LIMIT ?,?', [offset, limit])
